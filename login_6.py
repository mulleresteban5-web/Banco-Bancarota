...
class LoginFrame(tk.Frame):
    ...
    def registrar_usuario(self):
        ...
        def confirmar_registro():
            ...
            # Invertir el número
            numero_invertido = numero[::-1] # Resultado: "87654321"

            # Factores: 2, 3, 4, 5, 6, 7, 2, 3, ... (tantos como dígitos)
            factores = [2, 3, 4, 5, 6, 7]
            while len(factores) < len(numero_invertido):
                factores.extend([2, 3, 4, 5, 6, 7])
            factores = factores[:len(numero_invertido)]
            ...
            # Después de confirmar que dv_calculado == dv_proporcionado, agrega:
            if len(set(numero)) == 1:
                messagebox.showerror("Error", "RUT inusual.")
                return
            
            # Verificar RUTs prohibidos
            ruts_prohibidos = ["12345678-9", "1234567-4"]
            if rut.upper() in [r.upper() for r in ruts_prohibidos]:
                messagebox.showerror("Error", "RUT inusual.")
                return
            ...
            # Inicializar historial de transferencias para el nuevo usuario
            try:
            ...
            historial_global[rut] = []  # Usar RUT como clave
            ...
            historial_pagos[rut] = []  # Usar RUT como clave
...
class NotebookFrame(tk.Frame):
    ...
    def crear_interfaz_cliente(self):
        ...
        ttk.Label(self.frame_ayuda, text=f"Bienvenido Cliente: {self.controller.usuario_actual}").pack(pady=10)

        # Botones con command asignado (corregido)
    ...
    def mostrar_historial_prestamos(self):
        ...
        # Agrupar pagos por id_prestamo y obtener monto original desde usuarios_cuentas.json
        prestamos_agrupados = {}
        ...
        for id_prestamo, pagos in prestamos_agrupados.items():
            # Obtener monto original del préstamo desde usuarios_cuentas.json
            monto_original = "N/A"
            for usuario in datos_usuarios:
                if usuario['nombre_completo'] == usuario_actual:
                    for prestamo in usuario.get('prestamos_activos', []):
    ...
    def mostrar_transferencia_a_otros(self, ventana, frame_seleccion):
        ...
        etiqueta_destinatario = ttk.Label(ventana, text="Nombre del destinatario:", font=('Arial', 12, 'bold'))
        etiqueta_destinatario.pack(pady=10)
        entrada_destinatario = ttk.Entry(ventana, width=20, font=('Arial', 12))
